...
var timesArray = JSON.parse(loadLocal("megaPlltimesarray", "[]"));
...
function showScramble()
{
    ...
    if (window.selCases.length == 0) {
        s = "click \"select cases\" above and pick some Megaminx PLL to practice";
...
function displayPracticeInfo() {
    ...
    if (window.recapArray.length == 0)
        s += "Train mode: <b>" + window.selCases.length + "</b> case(s) selected";
    else
        s += "Recap mode: <b>" + window.recapArray.length + "</b> case(s) left";
...
function generateScramble()
{
    if (window.lastScramble != "")
        document.getElementById("last_scramble").innerHTML = "last scramble: " + window.lastScramble +
            " <span onclick='displayBox(event,"+lastCase+")' class='megaPllNameStats'>("+algsInfo[lastCase]["name"] + ") </span><a class='notObviousButton' onclick='confirmUnsel("+lastCase+")' style='color:"+document.getElementById( "linkscolor_in" ).value+";'>unselect</a>";
    displayPracticeInfo();
    // get random case
    var caseNum = 0;
    ...
    var alg = randomElement(window.megaPllMap[caseNum]);
    var bauf = randomElement(["", "U ", "U2 ", "U' ","U2' "]);
    var eauf = randomElement(["", " U", " U2", " U'"," U2'"]);
    var finalAlg = bauf + alg + eauf;
...
function displayBox(event,i) {
    ...
    document.getElementById("boxsetup").innerHTML = "Setup: " + window.megaPllMap[i][0];
    //document.getElementById("boxsetup").innerHTML = "Setup: " + inverse_scramble(algsInfo[i]["a"]);
    document.getElementById("boxImg").src="pic/"+algsInfo[i]["name"]+".png";
}
...
{
    saveLocal("megaPlltimesarray", JSON.stringify(window.timesArray));
    ...
    if (displayByCases)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            var currentMegaPll = window.timesArray[i]["case"];
            if (resultsByCase[currentMegaPll] == null)
                resultsByCase[currentMegaPll] = [];
            resultsByCase[currentMegaPll].push( window.timesArray[i] );
        }
        ...
        for (var j = 0; j < keys.length; j++) {
            ...
            var meanForCase = 0.0;
            var i = 0;
            for (; i < resultsByCase[megaPll].length; i++)
            {
                timesString += makeHtmlDisplayableTime(resultsByCase[megaPll][i]);
                if (i != resultsByCase[megaPll].length - 1)
                    timesString += ", ";
                // avg
                meanForCase *= i/(i+1);
                meanForCase += resultsByCase[megaPll][i]["ms"] / (i+1);
            }
            s += "<div class='megaPllNameHeader' " +
            ")'><span class='megaPllNameStats' onclick='displayBox(event,"+keys[j]+")'>" + algsInfo[megaPll]["name"] + "</span>: " + msToHumanReadable(meanForCase) + "</div>" + timesString + "<br><br>";
        }
        el.innerHTML = s;
    }
    else
...
function resetStyle(dark) {
    ...
    document.getElementById("linkscolor_in").value = dark ? "#ffff00" : "black";
    applystyle();
    savestyle();
}
